...
function MfaSignInStep({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // the auth store flips to signed-in and the panel re-renders
    } catch (err) {
      setError(friendlyMfaError(err));
    } finally {
      setBusy(false);
...
/**
 * The full account surface — profile, verification, 2FA, sync — shared by the
 * /account page and the sidebar sign-in modal. `onClose` is called after
 * actions that should dismiss a modal host (sign out, navigate away); a page
 * host simply omits it.
 */
export default function AccountPanel({ onClose }: { onClose?: () => void }) {
  ...
  // panel remounts with fresh state on every modal open / page visit — just
  // re-read the profile so a verification completed in another tab (or from
  // the email itself) shows up in the badge
  useEffect(() => {
    void refreshUser();
  }, []);
